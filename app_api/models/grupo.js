...
module.exports = function(sequelize, DataTypes) {
  ...
  Grupo.obtenerGrupoPorIdP = (idGrupo) => {
  ...
  */
  Grupo.crearGrupoT = (grupo, transaction) => {
    return new Promise( (resolve, reject) => {
      return Grupo.create({
        nombre         : grupo.nombre,
        tipo           : grupo.tipo,
  ...
  Grupo.editarGrupoT = (grupo, idGrupo, transaction) => {
    return new Promise( (resolve, reject) => {
      if ( !idGrupo )    { return reject( errors.SEQUELIZE_FK_ERROR('No ingresó el id del grupo') ); }
      if ( idGrupo < 0 ) { return reject( errors.SEQUELIZE_FK_ERROR('Id del grupo inválido') ); }
      return Grupo.update({
  ...
  /**
    ...
    @Error:
      {Object} fail Sequelize error {tipo, mensaje}
  */
  Grupo.eliminarGrupoT = (idGrupo, transaction) => {
    return new Promise( (resolve, reject) => {
      if ( !idGrupo )    { return reject( errors.SEQUELIZE_FK_ERROR('No ingresó el id del grupo') ); }
      if ( idGrupo < 0 ) { return reject( errors.SEQUELIZE_FK_ERROR('Id del grupo inválido') ); }
