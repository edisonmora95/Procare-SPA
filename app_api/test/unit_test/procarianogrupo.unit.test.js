	...
	describe('anadirProcarianoAGrupoT', () => {
      ...
      .then((resultado) => {
        console.log(resultado)
	  	...
	  	.catch( fail => {
	  		transaction.rollback();
				assert.equal(fail.tipo, 'Foreign key constraint error','Tipo de error recibido incorrecto');
				assert.equal(fail.mensaje, 'ER_NO_REFERENCED_ROW_2: procarianogrupo_ibfk_1',  'Mensaje recibido incorrecto');
				done();
			});
	  });
	  ...
	  it('CP7. Procariano no existe', done => {
	  	...
	  	.catch( fail => {
	  		transaction.rollback();
				assert.equal(fail.tipo, 'Foreign key constraint error','Tipo de error recibido incorrecto');
				assert.equal(fail.mensaje, 'ER_NO_REFERENCED_ROW_2: procarianogrupo_ibfk_2',  'Mensaje recibido incorrecto');
				done();
			});
	  });
