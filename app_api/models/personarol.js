...
module.exports = function(sequelize, DataTypes) {
  ...
  */
  PersonaRol.buscarRolesDePersonaPorId = (idPersona) => {
  ...
  */
  PersonaRol.personaTieneRolActualmente = (idPersona, rol) => {
    return new Promise( (resolve, reject) => {
      if( !idPersona )     return reject( errors.SEQUELIZE_FK_ERROR('No ingresó el id') );
      if( idPersona < 0 )  return reject( errors.SEQUELIZE_FK_ERROR('Id inválido') );
      return PersonaRol.findOne({
        where: {
          fechaFin  : null,
          PersonaId : idPersona,
          RolNombre : rol
  ...
  */
  PersonaRol.asignarRolT = (idPersona, rol, transaction) => {
    return new Promise( (resolve, reject) => {
      if( !idPersona )     return reject( errors.SEQUELIZE_FK_ERROR('No ingresó el id') );
      if( idPersona < 0 )  return reject( errors.SEQUELIZE_FK_ERROR('Id inválido') );
      return PersonaRol.create({
