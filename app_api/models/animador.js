...
module.exports = function(sequelize, DataTypes) {
  ...
  Animador.associate = (models) => {
    Animador.belongsTo(models.Procariano);
  }
  Animador.obtenerAnimadorDeGrupoP = (idGrupo) => {
  ...
  */
  Animador.animadorTieneGrupoActualmente = (idAnimador) => {
    return new Promise( (resolve, reject) => {
  ...
  Animador.obtenerAnimadoresActuales = () => {
    return new Promise((resolve, reject) => {
      return Animador.findAll({
        where : { fechaFin : null }
      })
      .then( registros => {
  ...
  /**
    ...
    @Error:
      {Object} fail Sequelize error {tipo, mensaje}
  */
  Animador.agregarAnimadorAGrupoT = (idAnimador, idGrupo, transaction) => {
    return new Promise( (resolve, reject) => {
      if( !idAnimador )     { return reject( errors.SEQUELIZE_FK_ERROR('No ingresó el id del animador') ); }
      if ( idAnimador < 0 ) { return reject( errors.SEQUELIZE_FK_ERROR('Id del animador inválido') ); }
      if( !idGrupo )        { return reject( errors.SEQUELIZE_FK_ERROR('No ingresó el id del grupo') ); }
      if ( idGrupo < 0 )    { return reject( errors.SEQUELIZE_FK_ERROR('Id del grupo inválido') ); }
      return Animador.create({
        ProcarianoId: idAnimador,
        GrupoId     : idGrupo,
  ...
  /**
    ...
    @Error:
      {Object} fail Sequelize error {tipo, mensaje}
  */
  Animador.cambiarAnimadorDeGrupoT = (idGrupo, idAnimadorAntiguo, idAnimadorNuevo, transaction) => {
    return new Promise( (resolve, reject) => {
      if( !idGrupo )               { return reject( errors.SEQUELIZE_FK_ERROR('No ingresó el id del grupo') ); }
      if ( idGrupo < 0 )           { return reject( errors.SEQUELIZE_FK_ERROR('Id del grupo inválido') ); }
  ...
  /**
    ...
    @Error:
      {Object} fail Sequelize error {tipo, mensaje}
  */
  Animador.eliminarRegistrosDeGrupoT = (idGrupo, transaction) => {
    return new Promise( (resolve, reject) => {
      if( !idGrupo )     { return reject( errors.SEQUELIZE_FK_ERROR('No ingresó el id del grupo') ); }
      if ( idGrupo < 0 ) { return reject( errors.SEQUELIZE_FK_ERROR('Id del grupo inválido') ); }
      return Animador.destroy({
        where : {
          GrupoId : idGrupo
        },
        transaction : transaction
