...
module.exports = function(sequelize, DataTypes) {
  ...
  */
  GrupoEtapa.crearGrupoEtapaT = (idGrupo, idEtapa, transaction) => {
  ...
  GrupoEtapa.cambiarGrupoDeEtapaT = (idGrupo, idEtapaAntigua, idEtapaNueva, transaction) => {
    return new Promise( (resolve, reject) => {
      if ( !idGrupo )    return reject( errors.SEQUELIZE_FK_ERROR('No ingresó el id del grupo') );
      if ( idGrupo < 0 ) return reject( errors.SEQUELIZE_FK_ERROR('Id del grupo inválido') );
  ...
  /**
    ...
    @Error:
      {Object} fail Sequelize error {tipo, mensaje}
  */
  GrupoEtapa.eliminarRegistrosDeGrupoT = (idGrupo, transaction) => {
    return new Promise( (resolve, reject) => {
      if ( !idGrupo )    return reject( errors.SEQUELIZE_FK_ERROR('No ingresó el id del grupo') );
      if ( idGrupo < 0 ) return reject( errors.SEQUELIZE_FK_ERROR('Id del grupo inválido') );
      return GrupoEtapa.destroy({
        where : {
          GrupoId : idGrupo
        },
        transaction : transaction
