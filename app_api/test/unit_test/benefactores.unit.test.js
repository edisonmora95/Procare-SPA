  ...
  describe('crearBenefactorT', () => {
    ...
    it('CP3. valorContribucion null', function(done) {
      ...
      .catch( fail => {
        transaction.rollback();
        assert.equal(fail.tipo, 'Validation error', 'Tipo de error incorrecto');
        assert.equal(fail.mensaje, 'Benefactor.valorContribucion cannot be null', 'Mensaje de error incorrecto');
        done();
      });
    });
    ...
    it('CP6. diaCobro null', function(done) {
      ...
      .catch( fail => {
        transaction.rollback();
        assert.equal(fail.tipo, 'Validation error', 'Tipo de error incorrecto');
        assert.equal(fail.mensaje, 'Benefactor.diaCobro cannot be null', 'Mensaje de error incorrecto');
        done();
      });
    });
    ...
    it('CP11. estado es null', function(done){
      ...
      .catch( fail => {
        transaction.rollback();
        assert.equal(fail.tipo, 'Validation error', 'Tipo de error incorrecto');
        assert.equal(fail.mensaje, 'Benefactor.estado cannot be null', 'Mensaje de error incorrecto');
        done();
      });
    });
